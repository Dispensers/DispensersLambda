	...
	constructor() {
		...
		for (let i = 0; i < puzzleSpecs.length; i++) {
			...
			puzzleRef.style =
				...
				"box-sizing:border-box; width:16.66%; " +
...
class MainWall {
	constructor(mainWallSpec, punterPuzzle) {
		...
		for (let d = 1; d <= punterPuzzle.numDispensers; d++) {
			...
			//Cosmetic shows automatically when dispenser configuration is enabled
...
class Punter {
	constructor(puzzle) {
		...
		this.matrix = new Matrix(puzzle, "#mwdpmDigit-", "#mwdpmTile-");
		...
		this.solveBiz = new SolveBiz(puzzle, this.dispensers, this.matrix, this.solveIO);
	}
...
class Demo {
	constructor() {
		const puzzleSpec = {
			...
			dispenserSpec: [undefined, "rps", "rt", "q", "spt"],
			matrixSpec: [9, 8, 7, -2, -1, 6, -3, -4, 5],
			colourSpec: ["Green", "GGBBGGBB", "Blue", "Orange", "Pink"],
			hintSpec: ["Blue", 3],
			solutionDispenseSequence: [4, 1, 1, 4, 3, 2, 2, 1, 4],
			solutionColourSequence: ["Orange", "Blue", "Blue", "Orange", "Pink", "Pink", "Green", "Green", "GGBBGGBB"]
		};
		...
		this.matrix = new Matrix(this.puzzle, "#iwdpmDigit-", "#iwdpmTile-");
		...
		this.solveBiz = new SolveBiz(this.puzzle, this.dispensers, this.matrix, this.solveIO);
	}
